...
# Create DATABASE
RUN mysql -u toor -proot -e CREATE DATABASE IF NOT EXISTS DSPARTYMANAGEMENT;

# Create Pool
RUN asadmin create-jdbc-connection-pool --restype java.sql.Driver --driverclassname com.mysql.jdbc.Driver --property 
'user=root:password=toor:URL="jdbc:mysql://localhost:3306/DSPARTYMANAGEMENT"' DSPARTYMANAGEMENT 

# Create Resource
RUN asadmin create-jdbc-resource --connectionpoolid jdbc/partydb DSPARTYMANAGEMENT 

# Deploy the application
RUN asadmin deploy --contextroot DSPartyManagement --name DSPartyManagement target/party.war  
