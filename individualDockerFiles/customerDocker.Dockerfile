...
RUN apt-get update && apt-get install -y python2.7 git maven curl
...
# Create DATABASE
RUN mysql -u toor -proot -e CREATE DATABASE IF NOT EXISTS DSCUSTOMER;

# Create Pool
RUN asadmin create-jdbc-connection-pool --restype java.sql.Driver --driverclassname com.mysql.jdbc.Driver --property 
'user=root:password=toor:URL="jdbc:mysql://localhost:3306/DSCUSTOMER"' DSCUSTOMER 

# Create Resource
RUN asadmin create-jdbc-resource --connectionpoolid jdbc/customerdbv2 DSCUSTOMER 

# Deploy the application
RUN asadmin deploy --contextroot DSCustomerManagement --name DSCustomerManagement target/customer.war  
