...
# Create DATABASE
RUN mysql -u toor -proot -e CREATE DATABASE IF NOT EXISTS DSPRODUCTINVENTORY;

# Create Pool
RUN asadmin create-jdbc-connection-pool --restype java.sql.Driver --driverclassname com.mysql.jdbc.Driver --property 
'user=root:password=toor:URL="jdbc:mysql://localhost:3306/DSPRODUCTINVENTORY"' DSPRODUCTINVENTORY 

# Create Resource
RUN asadmin create-jdbc-resource --connectionpoolid jdbc/pidbv2 DSPRODUCTINVENTORY 

# Deploy the application
RUN asadmin deploy --contextroot DSProductInventory --name DSProductInventory target/productInventory.war  
