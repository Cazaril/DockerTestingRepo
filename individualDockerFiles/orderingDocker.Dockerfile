...
# Create DATABASE
RUN mysql -u toor -proot -e CREATE DATABASE IF NOT EXISTS DSPRODUCTORDERING;

# Create Pool
RUN asadmin create-jdbc-connection-pool --restype java.sql.Driver --driverclassname com.mysql.jdbc.Driver --property 
'user=root:password=toor:URL="jdbc:mysql://localhost:3306/DSPRODUCTORDERING"' DSPRODUCTORDERING 

# Create Resource
RUN asadmin create-jdbc-resource --connectionpoolid jdbc/podbv2 DSPRODUCTORDERING 

# Deploy the application
RUN asadmin deploy --contextroot DSProductOrdering --name DSProductOrdering target/productOrdering.war  
